...
function makeSquareNumbers(arr) {
  const squareArray = arr.map((element) => element ** 2);
  return squareArray;
}
...
function filterForLargeWords(arr, length) {
  const filterArray = arr.filter((element) => element.length > length);
  return filterArray;
}
...
function findTheFirstEvenNumber(arr) {
  const target = arr.find((item) => item % 2 === 0);
  return target;
}
...
function sortCaseSensitive(arr) {
  ...
  sortedArray.sort((firstWord, secondWord) =>
...
function makeMirrorArray(arr) {
  const copyArray = [...arr];
  copyArray.pop();
  const mirrorArray = [...arr].reverse();
  return copyArray.concat(mirrorArray);
}
...
function dropRight(arr, n) {
  ...
  return sliceArray.slice(0, sliceArray.length - n);
}
...
function dropLeft(arr, n) {
  const sliceArray = [...arr];
  const range = sliceArray.length - n;
  return sliceArray.slice(-range);
}
